...
if (mainInput.innerText != '$000' && localStorage.getItem('balance') > 0 && localStorage.getItem('balance') >= inputBet) {
    ...
    tile.forEach(element => {
      element.classList.remove('clicked', 'thisBomb', 'notBomb')
      element.innerText = ''
...
takeBtn.addEventListener('click', ()=> {
  if (tileCount == 0 || isntBomb == false) {
   return 
  }
  localStorage.setItem('balance', balanceValue + inputBet * currentMultiply.innerText.substring(1))
  balance.innerHTML = localStorage.getItem('balance')
  bigBombCount.style.display = 'flex';
  mainBottom.style.visibility = 'visible';
  takeBtn.style.visibility = 'hidden'
  tile.forEach(element => {
